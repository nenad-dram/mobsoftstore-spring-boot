...
class ApplicationTest {
    ...
    void validateArchiveTest() {
        ...
        try (FileInputStream fis = new FileInputStream(fileSource)) {
            ...
            Assertions.assertTrue(
                    archiveInspector.isArchiveValid(entryMap, appName, appArchiveName));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    void defaultPictureTest() {
        try {
            byte[] smallImage =
                    archiveInspector.getDefaultImageByName(ArchiveInspector.PICTURE_128_NAME);
            byte[] bigImage =
                    archiveInspector.getDefaultImageByName(ArchiveInspector.PICTURE_512_NAME);
