...
public class ArchiveInspector {
    ...
    public static final String DEFAULT_PICTURE_128 = "default_logo/default_128.jpg";
    public static final String DEFAULT_PICTURE_512 = "default_logo/default_512.jpg";
    ...
    /**
     * Returns names for the two logo images specified within the txt file. Result is a map with
     * property's name as a key (either "picture_128" or "picture_512") and image's file name as a
     * value.
     *
            ...
            if (entry.getKey().endsWith(PROPERTIES_FILE_EXTENSION)) {
                ...
                try (InputStream is = new ByteArrayInputStream(entry.getValue());
                    ...
                    while ((line = br.readLine()) != null) {
                        ...
                        if (line.startsWith(PICTURE_128_NAME) || line.startsWith(
                                PICTURE_512_NAME)) {
    ...
    /**
     * Returns contents (bytes) for the two logo images within a map (key - image name, value -
     * image content). Result is based on the image name map and the archive content. If the image
     * is missing the default one will be used instead.
     *
     * @param imageNameMap images name map
     * @param entryMap     archive's content
     * @return logo images content map
     */
    public Map<String, byte[]> getImageContentMap(Map<String, String> imageNameMap,
    ...
    /**
     * Returns validation result for the given archive content. The archive is valid if it has a
     * {@value #PROPERTIES_FILE_EXTENSION} file and values for both {@value #APP_NAME_NAME} and
     * {@value #APP_PACKAGE_NAME} are the same as the values entered on the input form
     *
     * @param entryMap       archive entry map
     ...
     * @throws IOException if an I/O error occurs
     */
    public boolean isArchiveValid(Map<String, byte[]> entryMap, String appName,
            String appArchiveName) throws IOException {
            ...
            if (entry.getKey().endsWith(PROPERTIES_FILE_EXTENSION)) {
                ...
                try (InputStream is = new ByteArrayInputStream(entry.getValue());
                    ...
                    while ((line = br.readLine()) != null) {
                        ...
                        if (line.startsWith(APP_NAME_NAME) && appName.equals(
                                line.split(":")[1].trim())) {
                            hasValidName = true;
                        }
    ...
    /**
     * Returns default image's content for big or small image. The input parameter (imageName) is
     * name of the image parameter in the txt file -
     *
    ...
    public byte[] getDefaultImageByName(String imageName) throws IOException {
        byte[] content;
        String fileName =
                PICTURE_128_NAME.equals(imageName) ? DEFAULT_PICTURE_128 : DEFAULT_PICTURE_512;
